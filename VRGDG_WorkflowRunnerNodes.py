...
def _build_minimax_h3_2pass_api_prompt(payload):
    ...
    # NVENC / master extras that the VHS format spec does not declare and so
    # must be passed explicitly to the VideoCombine node:
    # - NVENC (GPU) encoders are bitrate-based: 'bitrate' (Mbit/s) + 'megabit'.
    # - AV1 WebM / FFV1 MKV are 10-bit masters on the 16-bit frame path:
    #   'input_color_depth' = '16bit' (a mandatory format widget, not in spec).
    if output_format.startswith("video/nvenc"):
        _set_api_input(prompt, "142", "bitrate", _int_payload(payload, "output_bitrate", 10, 1, 999))
        _set_api_input(prompt, "142", "megabit", True)
    elif output_format in {"video/av1-webm", "video/ffv1-mkv"}:
